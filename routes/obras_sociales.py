...
def init_app(app):
    ...
    @app.route('/api/obras-sociales/medicos/<int:med_id>/historico')
    @_api_safe
    def api_os_medico_historico(med_id):
        """Histórico 12 meses del médico: importe/recetas por mes + top OS + top productos."""
        from datetime import datetime as _dt
        from datetime import timedelta as _td
        from sqlalchemy import text as _text
        import database
        ...
        with database.get_db() as session:
            # Datos del médico
            med = session.execute(
                _text('SELECT om.observer_id, om.nombre, '
                      '(SELECT MIN(matricula) FROM obs_medicos_matriculas WHERE medico_observer = om.observer_id) AS matricula '
            ...
            sql_mes = _text('''
                ...
                FROM obs_ventas_detalle ovd
                WHERE ovd.medico_observer = :id
                  AND ovd.fecha_estadistica >= :desde
                  AND COALESCE(ovd.es_venta_particular, FALSE) = FALSE
                GROUP BY DATE_TRUNC('month', ovd.fecha_estadistica)
                ORDER BY mes
            ''')
            rows_mes = session.execute(sql_mes, {'id': med_id, 'desde': desde_12m}).fetchall()
            serie_mes = [{
                'mes': r.mes.isoformat(),
            ...
            sql_dia = _text('''
                ...
                ORDER BY fecha
            ''')
            rows_dia = session.execute(sql_dia, {'id': med_id, 'desde': desde_30d}).fetchall()
            serie_dia = [{
            ...
            sql_os = _text('''
                ...
                WHERE ovd.medico_observer = :id
                  AND ovd.fecha_estadistica >= :desde
                  AND COALESCE(ovd.es_venta_particular, FALSE) = FALSE
                GROUP BY oos.descripcion
                ORDER BY SUM(ovd.importe) DESC
                LIMIT 10
            ''')
            rows_os = session.execute(sql_os, {'id': med_id, 'desde': desde_12m}).fetchall()
            top_os = [{
                'nombre': r.os_nombre,
                'importe': float(r.importe or 0),
                'recetas': int(r.recetas or 0),
            ...
            sql_prod = _text('''
                ...
                LIMIT 10
            ''')
            rows_prod = session.execute(sql_prod, {'id': med_id, 'desde': desde_12m}).fetchall()
            top_prod = [{
                'descripcion': r.prod_nombre,
                'importe': float(r.importe or 0),
        ...
        return jsonify({
            'medico': {'id': med.observer_id, 'nombre': med.nombre, 'matricula': med.matricula},
