...
Antes el "armado del dict de display de un flag" (mapa de colores + assembly de
{slug, nombre, icono, color_clases, nota, ean_reemplazo}) estaba duplicado inline
en routes/productos.py y routes/compras_dia.py — dos bloques casi idénticos de
~40 líneas. Acá vive UNA sola vez.

Distinto de `obtener_flags_bulk` (services/pedido_estacional.py), que devuelve
los objetos crudos (ProductoFlag, TipoPedidoConfig) para el cálculo. Este módulo
es la capa de PRESENTACIÓN: el dict listo para el chip de la UI.
"""
import json
