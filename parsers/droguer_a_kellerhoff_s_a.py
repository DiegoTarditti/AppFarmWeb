...
def parse_invoice_pdf(pdf_path):
    ...
    proveedor_razon = razon_m.group(1).strip() if razon_m else 'DROGUERÍA KELLERHOFF S.A.'
    ...
    # Productos gravados: solo barcode cant descripcion importe
    ...
    for m in item_re.finditer(full_text):
        ...
        try:
            dto = round(float(m.group(5).replace(',', '.')), 2)
        except Exception:
            dto = None
        items.append({
