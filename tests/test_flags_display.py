...
def test_construir_flag_dict_con_cfg(session_demo):
    ...
    assert d['efecto_armado'] == 'badge_cero'  # del config_json del tipo


def test_construir_flag_dict_efecto_default(session_demo):
    """Sin efecto_armado en el config → 'ninguno' (default)."""
    s = session_demo
    cfg = s.query(TipoPedidoConfig).filter_by(slug='NOTA').first()  # sin efecto_armado
    flag = ProductoFlag(flag_slug='NOTA', ean='1')
    assert construir_flag_dict(flag, cfg)['efecto_armado'] == 'ninguno'
